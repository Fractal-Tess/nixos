...
exports.agentStatusController = agentStatusController;
const config_1 = require("../../config");
async function agentStatusController(req, res) {
    ...
    try {
        ...
        if (declaredLength > 1024 * 1024) throw new Error("Local agent status response exceeded 1 MiB");
        const text = await response.text();
        if (Buffer.byteLength(text, "utf8") > 1024 * 1024) throw new Error("Local agent status response exceeded 1 MiB");
        let body;
        try {
