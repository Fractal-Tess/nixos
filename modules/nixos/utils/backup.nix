...
let
  ...
  mkRestoreScript = { name, dataPaths, user, group, backupConfig }:
    ...
    in {
      ...
      script = ''
                # Create restore script directory
                mkdir -p /usr/local/bin

                # Create the restore script
                cat > /usr/local/bin/${name}-restore.sh << 'EOF'
                #!/bin/bash
                set -euo pipefail
        ...
        EOF
                ...
                chmod +x /usr/local/bin/${name}-restore.sh
                
                echo "Restore script created: /usr/local/bin/${name}-restore.sh"
      '';
    };
